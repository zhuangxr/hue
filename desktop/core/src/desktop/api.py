...
def _get_docs(user):
  ...
  docs = itertools.chain(
      Document.objects.get_docs(user).exclude(tags__in=[trash_tag])
        .filter(tags__in=[history_tag])
        .select_related('DocumentTag', 'User', 'DocumentPermission')
        .order_by('-last_modified')[:500],
      Document.objects.get_docs(user).exclude(tags__in=[history_tag])
        .select_related('DocumentTag', 'User', 'DocumentPermission')
...
def massaged_documents_for_json(documents, user):
  ...
  for document in documents:
    try:
      ...
      url = ''
    docs[document.id] = massage_doc_for_json(document, user)
...
def massage_doc_for_json(doc, user):
  ...
  permissions = massage_permissions(doc)
